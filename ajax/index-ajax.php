...
//If there are results for the query , they are shown.
    if($num_rows > 0) {
        while($row = $result->fetch_assoc()){
            ...
            $output['data'] .= "<tr>";                    
            $output['data'] .= "<td class='p-3'><a href='" . root . "recipes?recipe=" . $row['recipename'] . "&username=" . $_SESSION['username'] . "' title='receta' class='tlink'>" . $row['recipename'] . "</a></td>";
            $output['data'] .= "<td class='text-center'>" . $row['cookingtime'] . "</td>";
            $output['data'] .= "<td class='p-3'>" .ucfirst($row['category']). "</td>";            
            $output['data'] .= "<td class='p-3'>";
